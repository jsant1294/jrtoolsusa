      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: '24px', display: 'flex', gap: '8px', maxWidth: '500px' }}>
          <input
            ...
            style={{ flex: 1, padding: '10px 16px', border: '1px solid #e8e4dc', borderRadius: '4px', fontSize: '14px', fontFamily: "'Barlow', sans-serif", background: 'white', color: 'var(--navy)', outline: 'none' }}
          />
          {search && <button onClick={() => setSearch('')} style={btnStyle(false)}>Clear</button>}
        </div>

        <div style={{ marginBottom: '12px' }}>
          <div style={{ fontSize: '11px', fontWeight: 700, letterSpacing: '0.15em', textTransform: 'uppercase', color: 'var(--mid)', marginBottom: '8px' }}>Shop by Brand</div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
            <button onClick={() => setBrand('')} style={btnStyle(!brand)}>All</button>
            {BRANDS.map(b => (
              <button key={b} onClick={() => setBrand(brand === b ? '' : b)} style={btnStyle(brand === b, 'var(--red)')}>{b}</button>
            ))}
          </div>
        </div>

        <div style={{ marginBottom: '32px' }}>
          <div style={{ fontSize: '11px', fontWeight: 700, letterSpacing: '0.15em', textTransform: 'uppercase', color: 'var(--mid)', marginBottom: '8px' }}>Shop by Category</div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
            <button onClick={() => setCategory('')} style={btnStyle(!category)}>All</button>
            {CATEGORIES.map(c => (
              <button key={c.value} onClick={() => setCategory(category === c.value ? '' : c.value)} style={btnStyle(category === c.value, 'var(--red)')}>{c.label}</button>
            ))}
          </div>
        </div>

        {loading && (
          <div style={{ textAlign: 'center', padding: '64px 32px', color: 'var(--mid)' }}>Loading products...</div>
        )}
